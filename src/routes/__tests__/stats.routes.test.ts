...
describe('Stats API Endpoints', () => {
  ...
  describe('GET /api/stats/folders/:folderId/details', () => {
    ...
    beforeAll(async () => {
      ...
      // Create some review sessions for these sets by the user
      await prisma.userQuestionSetReview.createMany({
        data: [
          { userId, questionSetId: qs1Id, timeSpent: 600, sessionEndedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) }, // Renamed completedAt to sessionEndedAt and sessionDurationSeconds to timeSpent
          { userId, questionSetId: qs1Id, timeSpent: 700, sessionEndedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) }, // Renamed completedAt to sessionEndedAt and sessionDurationSeconds to timeSpent
          { userId, questionSetId: qs2Id, timeSpent: 500, sessionEndedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) }, // Renamed completedAt to sessionEndedAt and sessionDurationSeconds to timeSpent
        ],
      });
    });
